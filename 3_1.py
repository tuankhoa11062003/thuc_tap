...
def xu_ly_capcha(driver):
    ...
    # 2. Tiền xử lý ảnh bằng OpenCV để tăng độ chính xác
    img = cv2.imread("captcha.png", cv2.IMREAD_GRAYSCALE)
    _, thresh = cv2.threshold(img, 140, 255, cv2.THRESH_BINARY)  # Nhị phân hóa
    enlarged = cv2.resize(thresh, None, fx=2, fy=2, interpolation=cv2.INTER_LINEAR)  # Phóng to
    cv2.imwrite("processed.png", enlarged)

    # 3. OCR
    captcha_text = pytesseract.image_to_string(Image.open("processed.png"), config='--psm 8 --oem 3').strip()
